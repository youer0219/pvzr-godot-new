...
func _set_ladder_texure():
	...
	if has_up and not has_down:
		# 底端：有上无下
		region_rect.position = Vector2(1,0) * LevelData.SPRITE_CELL_SIZE
	elif not has_up and has_down:
		# 顶端：无上有下
		region_rect.position = Vector2(0,0) * LevelData.SPRITE_CELL_SIZE
	elif has_up and has_down:
		# 中间：有上有下
		region_rect.position = Vector2(3,0) * LevelData.SPRITE_CELL_SIZE
	elif not has_up and not has_down:
		# 孤立：无上无下
		region_rect.position = Vector2(2,0) * LevelData.SPRITE_CELL_SIZE
